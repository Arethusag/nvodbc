...
function ClientSession:connectToDatabase(dsn, uid, pwd)
    print("Connecting to the database..." .. dsn)
...
function parse_conn_str(conn_str)
    local args = {}

    -- Split the connection string into its components
    for component in string.gmatch(conn_str, "([^;]+)") do
        -- Format: key=value
        local key, value = string.match(component, "(.+)=(.-)")
        args[key] = value
    end
